...
const ActivitiesChart = () => {
  ...
  useEffect(() => {
    if (dayDistance === `1 month`) {
      fetch(`http://localhost:8080/activities/30?activist=${user?.email}`)
        .then((res) => res.json())
      ...
      fetch(`http://localhost:8080/activities/7?activist=${user?.email}`)
        .then((res) => res.json())
        .then((data) => setData(data));
      setShowChart(<WeekChart focus={focus} setTotal={setTotal} data={data} />);
    } else if (dayDistance === `1 year`) {
      fetch(`http://localhost:8080/activities/365?activist=${user?.email}`)
        .then((res) => res.json())
        .then((data) => setData(data));
      setShowChart(<YearChart focus={focus} setTotal={setTotal} data={data} />);
    } else {
      fetch(`http://localhost:8080/activities/1?activist=${user?.email}`)
        .then((res) => res.json())
        .then((data) => setData(data));
